...
class DynamodbHelper(Boto3Common):
    ...
    def get(self, match, total_retries=None, wait_int=None, raise_on_error=True):
        ...
        Raises an exception if a non-retryable error occurs and raise_on_error is True.
        """
        ...
        while True:
            try:
                ...
                self.logger.debug(
                    f'Retry dynamodb get retry={retry} - need to wait for thread to free up'
                )
                sleep(wait_int)
                retry += 1
    ...
    def search_key(self, key, value, total_retries=None, wait_int=None):
        """
        Searchs the DynamoDB table for an item with the specified key and value.
        ...
        while True:
            ...
            try:
                ...
                if err.response['Error']['Code'] not in self.retry_exceptions:
                    raise Exception(err.response['Error']) from err
                self.logger.debug(
                    f'Retry dynamodb search_key retry={retry} - need to wait for thread to free up'
                )
                sleep(wait_int)
                retry += 1
                if retry > total_retries:
    ...
    def search(self, total_retries=None, wait_int=None):
        ...
        while True:
            ...
            try:
                ...
                # Log the retry attempt and wait before retrying
                self.logger.debug(
                    f'Retry dynamodb search retry={retry} - need to wait for thread to free up'
                )
                sleep(wait_int)
                retry += 1
    ...
    def delete(self,match,total_retries=20,wait_int=None,raise_on_error=True):
        ...
        while True:
            ...
            try:
                ...
                if err.response['Error']['Code'] not in self.retry_exceptions:
                    ...
                    raise Exception(err.response['Error']) from err
                self.logger.debug(
                    f'Retry dynamodb delete retry={retry} - need to wait for thread to free up'
                )
                sleep(wait_int)
                retry += 1
                if retry > total_retries:
...
class Dynamodb_boto3(Boto3Common):
    ...
    def __init__(self,**kwargs):
        ...
        self.client: The Boto3 DynamoDB client.
        self.resource: The Boto3 DynamoDB resource.
