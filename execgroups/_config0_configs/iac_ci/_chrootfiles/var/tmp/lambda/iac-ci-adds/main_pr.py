...
class GitPr(PlatformReporter):
    ...
    def _get_s3_artifact(self,suffix_key):
        ...
        if file_info.get("status") is False:
            raise Exception(file_info["failed_message"])
...
'''
    ...
    def _update_comment(self,pr_md_info,existing_comments=None):
    ...
    def _clean_pr_comments(self):
        ...
        if status_comment_id:
            try:
                ...
                self.logger.debug(f"could not delete pr comment {status_comment_id}")
