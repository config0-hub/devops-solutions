...
class OrdersStagesHelper(Notification, CreateTempParamStoreEntry):
    ...
    def _set_infracost_api_key(self):
        ...
        try:
            _ssm_info = self.ssm.get_parameter(Name=self.trigger_info["ssm_infracost_api_key"],
                                               WithDecryption=True)
    ...
    def clean_pr_comments(self):
        ...
        for comment in comments:
            ...
            data_str = self.extract_from_substring(comment,
                                                   search_tag)
            if not data_str:
                continue
    ...
    def get_github_token(self):
        ...
        if not self.trigger_info:
            if items := self.db.get_trigger_info(repo_name=self.webhook_info["repo_name"]):
                self.trigger_info = self._filter_items_by_secret(items)
            else:
                return {
                    "status":False,
                    "failed_message": f'query is empty for {self.webhook_info["repo_name"]}'
                }

        if not self.trigger_info:
            return {"status":False}

        _ssm_info = self.ssm.get_parameter(Name=self.trigger_info["ssm_iac_ci_github_token"],
                                           WithDecryption=True)

        return _ssm_info["Parameter"]["Value"]
    ...
    def _filter_items_by_secret(self,items):
        """
        ...
        for item in items:
            ...
            if check_webhook_secret(secret, self.event_body, header_signature):
                self.logger.debug_highlight("signature validated")
                return item

        self.logger.error("Cannot verify signature")

        return

    def _set_trigger_info(self):
        """
        ...
        self.logger.debug(f'_set_trigger_info: trigger_id: {self.trigger_id}')
        self.logger.debug(f'_set_trigger_info: repo_name: {repo_name}')

        try:
            self.trigger_info = self.db.get_trigger_info(trigger_id=self.trigger_id,
    ...
    def set_s3_key(self):
        """
        self.remote_src_bucket_key = str(f"{self.stateful_id}/state/src.{self.stateful_id}.zip")
        ...
        self.local_src = str(f"/tmp/{commit_hash}.zip")
        self.remote_src_bucket_key = str(f"{self.stateful_id}/state/src.{commit_hash}.zip")
        self.remote_build_env_vars_key = str(f"{self.stateful_id}/state/{commit_hash}/build_env_vars.env.enc")
    ...
    def _setup(self):
        ...
        """
        set_info = self._set_trigger_info()
        ...
        self.ssm_callback_token = self.trigger_info["ssm_callback_token"]
        self.ssm_ssh_key = self.trigger_info["ssm_ssh_key"]
    ...
    def put_data_in_s3(self):
        ...
        self.s3_file.insert(s3_bucket=self.tmp_bucket,
                            s3_key=self.s3_key,
                            srcfile=srcfile)
    ...
    def _get_data_frm_s3(self):
        ...
        self.s3_file.get(s3_bucket=self.tmp_bucket,
                         s3_key=self.s3_key,
                         dstfile=dstfile)
    ...
    def _load_run_info(self):
        ...
        self.run_info = self.db.get_run_info(run_id=self.run_id,
    ...
    def _get_inputargs_http(self):
       ...
       """
        if not self.callback_token:
            _ssm_info = self.ssm.get_parameter(Name=self.ssm_callback_token,
                                               WithDecryption=True)
    ...
    def run(self,**kwargs):
        ...
        if hasattr(self,"init_failure") and self.init_failure:
            self.logger.warn("processing init_failure")
    ...
    def _init_build_env_vars(self):
        ...
        self.s3_file.get(s3_bucket=self.remote_src_bucket,
