...
class GitHubRepo:
    ...
    def get_pr_comments(self, use_default_search_tag=True, search_tag=None):
        ...
        while True:
            response = requests.get(self.url_issues_comments,
        ...
        if search_tag:
            ...
            return self._filter_comment_by_tag(comments,
    ...
    def _filter_comment_by_tag(self, comments, search_tag=None):
        ...
        Raises:
            Exception: If no search tag is provided.
        """
        if not search_tag:
            search_tag = self.search_tag

        if not search_tag:
            raise Exception("search tag is need to parse through comments")

        self.logger.debug(f"filter by search tag #{self.search_tag}")
        results = []

        if not self.search_tag:
            return False

        self.logger.debug(f"A total comments: {len(comments)}")

        # search the last line
        for comment in comments:
            if any(tag in comment['body'] for tag in [self.search_tag]):
                results.append(comment)

        self.logger.debug(f"search tag #{self.search_tag} found {len(results)}")
        return results
    ...
    def issue_to_pr(self, pr_number=None):
        ...
        Raises:
            Exception: If pr_number is not set.
        """
        if not pr_number:
            pr_number = self.pr_number

        if not pr_number:
            raise Exception("pr_number needs to be set")
    ...
    def _get_pr_review_comments(self):
        ...
        """
        _response = requests.get(self.url_pr_comment, headers=self.headers)

        if _response.status_code == 200:
            if _comments := _response.json():
                self.logger.debug(f"\nComments for Pull Request #{self.pr_number} in {self.repo_name}:")
                for _comment in _comments:
                    self.logger.debug(f"{_comment['user']['login']} on line {_comment['line']}: {_comment['body']}")
            else:
                self.logger.debug("No comments for this pull request.")
        else:
            self.logger.debug(f"Failed to fetch comments: {_response.status_code}, {_response.text}")
    ...
    def _add_pr_comment(self, comment):
        ...
        """
        return requests.post(self.url_issues_comments, headers=self.headers, json={'body': comment})
    ...
    def get_owner(self):
        ...
        Raises:
            Exception: If user info cannot be fetched.
        """
    ...
    def list_repos(self):
        ...
        Raises:
            Exception: If repositories cannot be fetched.
        """
    ...
    def _pr_comment_common(self, comment):
        ...
        Raises:
            ValueError: If 'GITHUB_PR_COMMENT_B64' is not set.
        """
        self._set_pr_urls()
    ...
    def update_pr_comment(self, comment_id, comment):
        ...
        if response.status_code == 200:
            ...
            self.logger.debug(f"Comment update successfully with ID: {comment_id}.")
            return {
    ...
    def add_pr_comment(self, comment):
        ...
        """
        encoded_comment, md5_hash = self._pr_comment_common(comment)
        comment = base64.b64decode(encoded_comment).decode('utf-8')

        response = self._add_pr_comment(comment)

        if response.status_code == 201:
            comment_data = response.json()  # Get the response data
            comment_id = comment_data['id']  # Extract the comment ID
    ...
    def delete_pr_comment(self, comment_id):
        ...
        if response.status_code == 204:
            ...
            self.logger.debug(f"Comment {comment_id} deleted failed.")
