...
class PkgCodeToS3(PlatformReporter):
    ...
    def _write_private_key(self):
        """
        _write_private_key method writes a private key to a file.
    ...
    def _fetch_code(self):
        """
        _fetch_code method fetches the source code from github.
    ...
    def _get_build_env_vars_frm_file(self):
        ...
        match_files = find_filename(self.archive_dir,
                                    'build_env_vars.env.enc')
        ...
        self.s3_file.insert(s3_bucket=self.remote_src_bucket,
    ...
    def _archive_code(self):
        ...
        shutil.make_archive(f'/tmp/{self.commit_hash}',
                            'zip',
                            verbose=True)
    ...
    def _save_run_info(self):
        """
        _save_run_info method saves the run_info to the table_runs in DynamoDB.
